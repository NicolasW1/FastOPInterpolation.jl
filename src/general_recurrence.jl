export AbstractRecurrenceBuffer
...
# """
@inbounds @inline function initalizeRecurrence!(buffer::AbstractRecurrenceBuffer, x::AbstractVector)::Nothing
    resetBuffer!(buffer)
    buffer.x .= x
...
# """
@inline @fastmath function jacobiRecurrenceRelation(Pₙ₋₁::T, Pₙ₋₂::T, n::Integer, α::T, β::T, x::T)::T where {T}
    if n==0
        one(T)
    elseif n==1
        (α - β + x * (2 + α + β))/2
